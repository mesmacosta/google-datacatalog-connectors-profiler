...
from google.datacatalog_connectors.greenplum import datacatalog_cli
...
if __name__ == "__main__":
    ...
    datacatalog_cli.Greenplum2DatacatalogCli().run()

    now = datetime.datetime.now()

    end_time = now.strftime('%m/%d/%y %H:%M:%S')

    sys.argv.extend(['--entry-group-id', 'greenplum'])
