...
def __delete_entries_and_groups(project_id):
    ...
    search_results = [
        result for result in __datacatalog.search_catalog(
            scope=scope, query=query, page_size=1000, timeout=300000)
    ]
...
def __delete_tag_templates(project_id):
    ...
    for result in search_results:
        try:
            __datacatalog.delete_tag_template(result.relative_resource_name,
                                              force=True)
            print('--> Tag Template deleted: {}'.format(
